...
%Usage: [roc,EER,area,EERthr,ALLthr,d,gen,imp]=ezroc3(H); for multi-class, use H=cat(3,H1,H2,...) for multiple H's
%Or  [roc,EER,area,EERthr,ALLthr,d,gen,imp,rbst]=ezroc3(H,T); (single-class-vs-rest)
%Or  [roc,EER,area,EERthr,ALLthr,d,gen,imp,rbst]=ezroc3(H,T,plot_stat), where plot_stat of 0 turns off plotting, and plot_stat=3 plots 3D ROC (y=GAR x=FAR z=thr)
...
function [roc,EER,area,EERthr,ALLthr,d,gen,imp,rbst]=ezroc3(H,T,plot_stat)
t1=min(min(min(H)));
t2=max(max(max(H)));
num_subj=size(H,1);
